...
from orm.interfaces import IQuery, IStatements, IRepositoryBase, ISelect
...
from orm.interfaces.ISelect import TableColumn
...
class AbstractSQLStatements[T: Table](IStatements[T]):
    ...
    @property
    @abstractmethod
    def INSERT_QUERY(self) -> Type[AbstractInsertQuery[T]]: ...
    ...
    @property
    @abstractmethod
    def DELETE_QUERY(self) -> Type[AbstractDeleteQuery[T]]: ...
    @property
    @abstractmethod
    ...
    def insert(self, values: T | list[T]) -> None:
        query, values = self.INSERT_QUERY(self._model).insert(values)
