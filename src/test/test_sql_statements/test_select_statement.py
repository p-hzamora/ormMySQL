...
class TestJoinQueries(unittest.TestCase):
    ...
    # TODOM []: Check why ForeignKey alias in 'a.B_fk_b1.CSimple' 'a.B_fk_b2.CSimple' it's not working as expected.
    def test_new_select_context(self):
        ...
        select = self.model.select(lambda x: (x.data_a, x.B_fk_b1.name,), flavour=tuple)
        print(select)
