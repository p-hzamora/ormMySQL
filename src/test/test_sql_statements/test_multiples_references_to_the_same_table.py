...
from ormlambda import Alias, IStatements
...
def create_tables(engine_no_db: Engine, create_engine_for_db: Callable[[str], Engine]):  # noqa: F811
    engine_no_db.create_schema(DDBBNAME, "replace")

    engine = create_engine_for_db(DDBBNAME)

    ORM(D, engine).create_table()
    ORM(C, engine).create_table()
    ORM(B, engine).create_table()
    ORM(A, engine).create_table()

    insert_data(engine)

    yield

    engine.drop_schema(DDBBNAME)
...
@pytest.fixture
def model(engine) -> IStatements[A]:
    return ORM(A, engine)
...
def test_new_select_context_using_pydantic(model: IStatements[A]):
...
def test_raise_DuplicatedClauseName_withous_usingalias_when_repeting_columns(model: IStatements[A]):
    with pytest.raises(DuplicatedClauseNameError) as err:
        model.where(lambda x: x.B3.C1.D2.name == "Developer").first(
            lambda a: (
                a.B3.C1.D2.name,
                a.B3.C2.D2.name,
                a.B3.C3.D2.name,
    ...
    assert err.value.names == ("name",)


# FIXME [x]: Check why ForeignKey alias in 'a.B_fk_b1.C' 'a.B_fk_b2.C' it's not working as expected.


def test_select_a_lot_of_columns_with_a_lot_of_combines(model: IStatements[A]):
    VARIABLE = 3
    res = model.where(lambda x: x.pk_a == VARIABLE).first(
        lambda a: (
...
def test_select_a_lot_of_columns_using_global_alias(model: IStatements[A]):
    VARIABLE = 3
    res = model.where(lambda x: x.pk_a == VARIABLE).first(
        lambda a: (
            a.pk_a,
