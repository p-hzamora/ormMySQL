...
def WhereMySQL(*args, **kwargs) -> Where:
    return Where(*args, **kwargs, dialect=DIALECT)
...
class TestWhere(unittest.TestCase):
    ...
    def test_one_where(self):
        w = WhereMySQL(Address.address == 10)
...
class TestCondition(unittest.TestCase):
    ...
    def test_to_query_cond_1(self):
        self.assertEqual(WhereMySQL(self.COND_CI_CO).query, "WHERE city.last_update != country.country_id")

    def test_to_query_cond_2(self):
        self.assertEqual(WhereMySQL(self.COND_A_CI).query, "WHERE address.address2 <= city.city_id")

    def test_to_query_cond_3(self):
        self.assertEqual(WhereMySQL(self.COND_B_C).query, "WHERE a.value = b.data")

    def test_join_condition_restrictive_false(self):
        joins = WhereMySQL(self.COND_CI_CO, self.COND_A_CI, self.COND_B_C, restrictive=False)
    ...
    def test_join_condition_restrictive_true(self):
        joins = WhereMySQL(self.COND_CI_CO, self.COND_A_CI, self.COND_B_C, restrictive=True)
    ...
    def test_value_replace(self):
        cond = WhereMySQL(Address.city_id == City.city_id)
    ...
    def test_value_replace_2(self):
        ...
        cond_2 = WhereMySQL((ADDRESS_1.city_id != City.city_id) & (City.city_id <= country.country_id))
        self.assertEqual(cond_2.query, "WHERE city.city_id != 1 AND city.city_id <= 50")

    # def test_like_condition(self):
    #     country = Country(50, "Espanna")

    #     cond_2 = WhereMySQL(Address.city_id, ConditionType.REGEXP, country.country)
    #     self.assertEqual(cond_2.query, "WHERE address.city_id REGEXP 'Espanna'")

    # def test_raise_KeyError(self):
    #     with self.assertRaises(KeyError):
    #         WhereMySQL((Address.city_id != City.city_id) & (City.city_id<= Country.country_id) & (Country.country_id == 3)).query

    def test_replace_address(self):
        cond = WhereMySQL(City.city == ADDRESS_1.address)
    ...
    def test_wrapped_distinct_from_string_const_content(self):
        cond = WhereMySQL(City.city > 1000)
        self.assertEqual(cond.query, "WHERE city.city > 1000")
