...
class TestWhere(unittest.TestCase):
    def test_one_where(self):
        ...
        self.assertEqual(w.compile(DIALECT).string, "WHERE `address`.address = 10")
    ...
    def test_multiples_where(self):
        ...
        w = Where(*cols)
        self.assertEqual(w.compile(DIALECT).string, "WHERE `address`.address = 'madrid' AND `address`.address2 = 'other' AND `address`.district = 1000 AND `address`.city_id = 87")
...
#         )
#         self.assertEqual(w.compile(DIALECT), "WHERE address.address = 'sol' AND city.city = 'Madrid'")
...
#         )
#         self.assertEqual(w.compile(DIALECT), "WHERE address.address = 'sol' AND city.city = 'Madrid' AND country.country = 'Spain'")

#     def test_where_with_regex(self):
#         pattern: str = r"^[A+]"
#         w = Where(Regex(Address.address, pattern))
#         self.assertEqual(w.compile(DIALECT), f"WHERE address.address REGEXP '{pattern}'")
...
#         self.assertEqual(w.compile(DIALECT), f"WHERE address.address LIKE '{pattern}'")
...
#         self.assertEqual(w.compile(DIALECT), "WHERE address.address_id IN (1, 2, 3, 4, 5, 6, 7, 8, 9)")

#     def test_where_not_contains(self):
#         w = Where(Address.address_id.not_contains((1, 2, 3, 4, 5, 6, 7, 8, 9)))
#         self.assertEqual(w.compile(DIALECT), "WHERE address.address_id NOT IN (1, 2, 3, 4, 5, 6, 7, 8, 9)")
...
#         self.assertEqual(Where(self.COND_CI_CO).compile(DIALECT), "WHERE city.last_update != country.country_id")

#     def test_to_query_cond_2(self):
#         self.assertEqual(Where(self.COND_A_CI).compile(DIALECT), "WHERE address.address2 <= city.city_id")

#     def test_to_query_cond_3(self):
#         self.assertEqual(Where(self.COND_B_C).compile(DIALECT), "WHERE a.value = b.data")

#     def test_join_condition_restrictive_false(self):
#         joins = Where(self.COND_CI_CO, self.COND_A_CI, self.COND_B_C, restrictive=False)
#         self.assertEqual(joins.compile(DIALECT), "WHERE city.last_update != country.country_id OR address.address2 <= city.city_id OR a.value = b.data")

#     def test_join_condition_restrictive_true(self):
#         joins = Where(self.COND_CI_CO, self.COND_A_CI, self.COND_B_C, restrictive=True)
#         self.assertEqual(joins.compile(DIALECT), "WHERE city.last_update != country.country_id AND address.address2 <= city.city_id AND a.value = b.data")

#     def test_value_replace(self):
#         cond = Where(Address.city_id == City.city_id)
#         self.assertEqual(cond.compile(DIALECT), "WHERE address.city_id = city.city_id")
...
#         self.assertEqual(cond_2.compile(DIALECT), "WHERE city.city_id != 1 AND city.city_id <= 50")

#     # def test_like_condition(self):
#     #     country = Country(50, "Espanna")

#     #     cond_2 = Where(Address.city_id, ConditionType.REGEXP, country.country)
#     #     self.assertEqual(cond_2.compile(DIALECT), "WHERE address.city_id REGEXP 'Espanna'")

#     # def test_raise_KeyError(self):
#     #     with self.assertRaises(KeyError):
#     #         Where((Address.city_id != City.city_id) & (City.city_id<= Country.country_id) & (Country.country_id == 3)).compile

#     def test_replace_address(self):
#         cond = Where(City.city == ADDRESS_1.address)
#         self.assertEqual(cond.compile(DIALECT), "WHERE city.city = 'Calle Cristo de la victoria'")

#     def test_wrapped_string_variable_content(self):
#         variable_ = "var_string"
#         cond = Where(City.city > variable_)
#         self.assertEqual(cond.compile(DIALECT), "WHERE city.city > 'var_string'")
...
#         variable_ = 1000
#         cond = Where(City.city > variable_)
#         self.assertEqual(cond.compile(DIALECT), "WHERE city.city > 1000")
...
#         )
#         self.assertEqual(cond.compile(DIALECT), "WHERE `address`.address_id >= 40 AND `address`.address_id < 100 AND `city`.city_id >= 30 AND `city`.city_id < 100 AND `country`.country_id >= 60 AND `country`.country_id < 100")
