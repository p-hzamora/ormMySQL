...
class TestSelect(unittest.TestCase):
    ...
    def test_AAAselect_with_context(self):
        context = ClauseInfoContext()
        select = Select[D](D, func.Concat((D.pk_d, "-", D.data_d), context=context), context=context)

        query = "SELECT CONCAT(d.pk_d, '-', d.data_d) AS `concat` FROM d AS `d`"
