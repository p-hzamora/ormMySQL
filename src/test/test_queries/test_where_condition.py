...
class TestCondition(unittest.TestCase):
    COND_CI_CO = Where[City, Country](instances=(City, Country), function=lambda x, y: x.last_update != y.country_id)
    COND_A_CI = Where[Address, City](instances=(Address, City), function=lambda a, c: a.address2 <= c.city_id)
    COND_B_C = Where[B, C](instances=(B, C), function=lambda b, c: b.A.value == c.B.data)

    def test_condition_constructor(self):
        self.assertIsInstance(self.COND_CI_CO, Where)
        self.assertIsInstance(self.COND_A_CI, Where)
        self.assertIsInstance(self.COND_B_C, Where)
    ...
    def test_join_condition_restrictive_false(self):
        joins = Where.join_condition(self.COND_CI_CO, self.COND_A_CI, self.COND_B_C, restrictive=False)
    ...
    def test_join_condition_restrictive_true(self):
        joins = Where.join_condition(self.COND_CI_CO, self.COND_A_CI, self.COND_B_C, restrictive=True)
    ...
    def test_value_replace(self):
        cond = Where[Address, City](instances=(Address, City), function=lambda a, ci: a.city_id == ci.city_id)
        self.assertEqual(cond.query, "WHERE address.city_id = city.city_id")

        city = City(1, "Madrid", 50)
        cond_2 = Where[Address, City](instances=(Address, City), function=lambda a, ci: a.city_id == ci.country_id, a=ADDRESS_1, ci=city)
        self.assertEqual(cond_2.query, "WHERE 1 = 50")

    def test_value_replace_2(self):
        country = Country(50, "Espanna")

        cond_2 = Where[Address, City, Country](
    ...
    def test_like_condition(self):
        ...
        cond_2 = Where[Address, Country](instances=(Address, Country), function=lambda a, c: (a.city_id, ConditionType.REGEXP, c.country), c=country)
        self.assertEqual(cond_2.query, "WHERE address.city_id REGEXP 'Espanna'")

    def test_raise_KeyError(self):
        with self.assertRaises(KeyError):
            Where[Address, City, Country](instances=(Address, City, Country), function=lambda a, ci, co: a.city_id != ci.city_id <= co.country_id == 3).query

    def test_replace_address(self):
        cond = Where[City, Address](instances=(City, Address), function=lambda c, a: c.city == a.address, a=ADDRESS_1)
    ...
    def test_wrapped_string_variable_content(self):
        ...
        cond = Where[City, Address](instances=(City,), function=lambda c: c.city > variable_, variable_=variable_)
        self.assertEqual(cond.query, "WHERE city.city > 'var_string'")

    def test_wrapped_string_const_content(self):
        cond = Where[City, Address](instances=(City,), function=lambda c: c.city > "var_string")
        self.assertEqual(cond.query, "WHERE city.city > 'var_string'")
    ...
    def test_wrapped_distinct_from_string_const_content(self):
        cond = Where[City, Address](instances=(City,), function=lambda c: c.city > 1000)
        self.assertEqual(cond.query, "WHERE city.city > 1000")

    def test_no_replace_variable(self):
        variable_ = "var_string"
        cond = Where[City, Address](instances=(City,), function=lambda c: c.city > variable_)
