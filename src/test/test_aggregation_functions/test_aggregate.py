...
class TestFunction(unittest.TestCase):
    ...
    def test_functions[T, TProp](self, where, function: Callable[[T], tuple[TProp, ...]], response) -> None:
        class Response(BaseModel):
            response: int
    ...
    def test_AAoperator(self):
        ...
        self.assertEqual(res.address,1)
        self.assertEqual(res.city,300)
        self.assertEqual(res.country,20)
        self.assertEqual(res.truncate,res.address + res.city + res.country)
        self.assertEqual(res.suma,res.truncate)
        # fmt:on
        pass
