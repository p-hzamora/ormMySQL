...
class DeferredOperation:
    ...
    def instantiate(self) -> AggregateFunctionBase:
        return self.cls_obj(*self.args, **self.kwargs)


class AliasedColumn:
    def __init__(self, column_ref: ColumnProxy, alias_name: str):
        self.column_ref = column_ref
        self.alias_name = alias_name

    def __str__(self):
        return f"{self.column_ref._get_full_reference()} AS {self.alias_name}"
...
class Statements[T: Table, TRepo](BaseStatement[T, None]):
    ...
    def count[TProp](
        ...
        selection = GlobalChecker.resolved_callback_object(selection, self.model)
    ...
    def where(self, conditions: WhereTypes) -> IStatements_two_generic[T, TRepo]:
        ...
        conditions = GlobalChecker.resolved_callback_object(conditions, self._model)
        if not isinstance(conditions, Iterable):
            conditions = (conditions,)

        deferred_op = DeferredOperation(clauses.Where, *conditions)
    ...
    def order[TValue](self, columns: Callable[[T], TValue], order_type: OrderTypes) -> IStatements_two_generic[T, TRepo]:
        deferred_op = DeferredOperation(clauses.Order, callback=columns, order_type=order_type, dialect=self._dialect)
        self._deferred_operations.append(deferred_op)
    ...
    def select[TValue, TFlavour, *Ts](
        ...
        **kwargs,
        ...
        with PATH_CONTEXT.query_context(self.model) as context:
            table_proxy = TableProxy(self.model, context.get_current_path())

            self._resolve_deferred_operations(table_proxy)

            for op in self._immediate_operations:
                self._query_builder.add_statement(op)

            select_clause = GlobalChecker.resolved_callback_object(selector, table_proxy)
    ...
    def _resolve_deferred_operations(self, table_proxy: TableProxy):
    ...
    def alias[TProp](self, column: SelectCols[T, TProp], alias: AliasType[ClauseInfo[T]]) -> clauses.Alias[T]:
        ...
        if isinstance(column, ColumnProxy):
            proxy = column._column
            alias_table = cast(ColumnProxy, column).get_alias()
            column = proxy

        return ClauseInfo(column.table, column=column, alias_table=alias_table, alias_clause=alias, dialect=self.dialect)
