...
from ..sql.sql_methods import SQLMethods
...
class Statements[T: Table, TRepo](BaseStatement[T, None]):
    def __init__(self, model: T, repository: BaseRepository[None], methods: SQLMethods) -> None:
        super().__init__(model, repository=repository)
        self._methods: SQLMethods = methods
    ...
    def insert(self, instances: T | list[T]) -> None:
        insert = self._methods.insert(self._model, self._repository)
    ...
    def delete(self, instances: Optional[T | list[T]] = None) -> None:
        ...
        delete = self._methods.delete(self._model, self._repository)
    ...
    def upsert(self, instances: T | list[T]) -> None:
        upsert = self._methods.upsert(self._model, self._repository)
    ...
    def update(self, dicc: dict[str, Any] | list[dict[str, Any]]) -> None:
        update = self._methods.update(self._model, self._repository, self._query_builder.WHERE)
        update.update(dicc)
        update.execute()
    ...
    def limit(self, number: int) -> IStatements_two_generic[T, TRepo]:
        limit = self._methods.limit(number)
    ...
    def offset(self, number: int) -> IStatements_two_generic[T, TRepo]:
        offset = self._methods.offset(number)
    ...
    def count[TProp](
        ...
        return self._methods.count(element=selection, alias_clause=alias, context=self._query_builder._context)
    ...
    def where(self, conditions: WhereTypes) -> IStatements_two_generic[T, TRepo]:
        ...
        self._query_builder.add_statement(self._methods.where(*conditions))
        return self
    ...
    def having(self, conditions: WhereTypes) -> IStatements_two_generic[T, TRepo]:
        ...
        if not isinstance(conditions, Iterable):
            conditions = (conditions,)
        self._query_builder.add_statement(self._methods.having(*conditions))
        return self
    ...
    def max[TProp](
        ...
        return self._methods.max(elements=column, alias_clause=alias, context=self._query_builder._context)
    ...
    def min[TProp](
        ...
        return self._methods.min(elements=column, alias_clause=alias, context=self._query_builder._context)
    ...
    def sum[TProp](
        ...
        return self._methods.sum(elements=column, alias_clause=alias, context=self._query_builder._context)
    ...
    def groupby[TProp](self, column: ColumnType[TProp] | Callable[[T], Any]) -> IStatements_two_generic[T]:
        ...
        groupby = self._methods.groupby(column=column, context=self._query_builder._context)
        # Only can be one LIMIT SQL parameter. We only use the last LimitQuery
        self._query_builder.add_statement(groupby)
        return self

    @override
    def alias[TProp](self, column: SelectCols[T, TProp], alias: AliasType[ClauseInfo[T]]) -> _Alias[T]:
        column = GlobalChecker.resolved_callback_object(column, self.models)
