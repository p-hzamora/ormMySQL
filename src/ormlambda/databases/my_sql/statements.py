...
class MySQLStatements[T: Table, *Ts](AbstractSQLStatements[T, *Ts, MySQLConnection]):
    ...
    @override
    def where(self, conditions: WhereTypes, **kwargs) -> IStatements_two_generic[T, MySQLConnection]:
    ...
    def join[FKTable](self, joins: tuple[*TupleJoinType[FKTable]]) -> JoinContext[tuple[*TupleJoinType[FKTable]]]:
        ...
        return JoinContext[T, tuple[*TupleJoinType[FKTable]]](self.model, joins)
    ...
    @override
    def select[TValue, TFlavour, *Ts](self, selector: Optional[tuple[TValue, *Ts]] = [], *, flavour: Optional[Type[TFlavour]] = None, by: JoinType = JoinType.INNER_JOIN, **kwargs):
        if len(selector) == 0:
            ...
            result = self.select(selector=self._model, flavour=flavour, by=by)
    ...
    @override
    def select_one[TValue, TFlavour, *Ts](self, selector: Optional[tuple[TValue, *Ts]] = [], *, flavour: Optional[Type[TFlavour]] = None, by: JoinType = JoinType.INNER_JOIN):
        self.limit(1)
        if len(selector) == 0:
