...
from ormlambda.sql.clauses import _Limit as Limit
from ormlambda.sql.clauses import _Offset as Offset
from ormlambda.sql.clauses import _Order as Order
from ormlambda.sql.clauses import _Select as Select

from ormlambda.sql.clauses import _Where as Where
from ormlambda.sql.clauses import _Having as Having
from ormlambda.sql.clauses import _GroupBy as GroupBy
...
class QueryBuilder(IQuery):
    __order__: tuple[str, ...] = ("_Select", "_JoinSelector", "_Where", "_GroupBy", "_Having", "_Order", "_Limit", "_Offset")
    ...
    def SELECT(self) -> IQuery:
        return self._query_list.get("_Select", None)

    @property
    def WHERE(self) -> IQuery:
        where = self._query_list.get("_Where", None)
    ...
    def ORDER(self) -> IQuery:
        return self._query_list.get("_Order", None)

    @property
    def GROUP_BY(self) -> IQuery:
        return self._query_list.get("_GroupBy", None)

    @property
    def HAVING(self) -> IQuery:
        where = self._query_list.get("_Having", None)
        if not isinstance(where, Iterable):
            if not where:
                return ()
            return (where,)
    ...
    def LIMIT(self) -> IQuery:
        return self._query_list.get("_Limit", None)

    @property
    def OFFSET(self) -> IQuery:
        return self._query_list.get("_Offset", None)
