...
class ClauseInfoConverter[T, TProp](tp.Protocol):
    @classmethod
    def convert(cls, data: T, alias_table: AliasType[ColumnType[TProp]] = "{table}", **kwargs) -> list[ClauseInfo[T]]: ...


class ConvertFromAnyType(ClauseInfoConverter[None, None]):
    @classmethod
    def convert(cls, data: tp.Any, alias_table: AliasType = "{table}", **kwargs) -> list[ClauseInfo[None]]:
        return [ClauseInfo[None](table=None, column=data, alias_table=alias_table, alias_clause=kwargs.get("alias", None), **kwargs)]


class ConvertFromForeignKey[LT: Table, RT: Table](ClauseInfoConverter[RT, None]):
    @classmethod
    def convert(cls, data: ForeignKey[LT, RT], alias_table=None, **kwargs) -> list[ClauseInfo[RT]]:
        return ConvertFromTable[RT].convert(data.tright, data.get_alias(kwargs["dialect"]), **kwargs)
...
class ConvertFromIAggregate(ClauseInfoConverter[None, None]):
    @classmethod
    def convert(cls, data: AggregateFunctionBase, alias_table=None, **kwargs) -> list[ClauseInfo[None]]:
        return [data]


class ConvertFromTable[T: Table](ClauseInfoConverter[T, None]):
    @classmethod
    def convert(cls, data: T, alias_table: AliasType[ColumnType] = "{table}", **kwargs) -> list[ClauseInfo[T]]:
        """
        if the data is Table, means that we want to retrieve all columns
        """
        return cls._extract_all_clauses(data, alias_table, **kwargs)

    @staticmethod
    def _extract_all_clauses(table: TableType[T], alias_table: AliasType[ColumnType], **kwargs) -> list[ClauseInfo[TableType[T]]]:
        # all columns
        column_clauses = []
        for column in table.get_columns():
            column_clauses.extend(ConvertFromColumn.convert(column, alias_table=alias_table, **kwargs))
