...
class ClauseInfo[T: Table](IQuery):
    ...
    def alias_clause(self) -> tp.Optional[str]:
        return self._clean_alias(self._alias_clause_resolver(self._alias_clause))
    ...
    def alias_table(self) -> tp.Optional[str]:
        return self._clean_alias(self._alias_table_resolver(self._alias_table))
...
class AggregateFunctionBase(ClauseInfo[None], IAggregate):
    ...
    def _create_query(self) -> str:
        ...
        return self._concat_alias_and_column(f"{self.FUNCTION_NAME()}({self._column})", alias_clause)
